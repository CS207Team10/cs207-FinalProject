...
path = os.path.dirname(__file__) +  "/data/xml/"
...
def test_parse_reactionList():
    reactionList = chemkin.ChemUtil.parse(path + "rxns_short_2.xml", 340, 8.314)[0]
...
def test_parse_species():
    species = chemkin.ChemUtil.parse(path + "rxns_short_2.xml", 340, 8.314)[1]
    assert(species[0] == "H2")
    assert(len(species) == 5)

def test_parse_Not_XML():
    try:
        chemkin.ChemUtil.parse(path + "rxns_short_2.pdf", 340, 8.314)
    except ValueError as err:
        assert(type(err) == ValueError)

def test_parse_T_neg():
    try:
        chemkin.ChemUtil.parse(path + "rxns_short_2.xml", -340, 8.314)
    except ValueError as err:
        assert(type(err) == ValueError)
